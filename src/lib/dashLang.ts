...
const id = {
  ...
  // KPIs
  k_panasonicSales: "Panasonic Sales", k_panasonicSalesSub: "SPOS · Siap Dikirim",
  k_totalGmv: "Total GMV", k_totalGmvSub: "Performa",
  k_panaTraffic: "Pana Traffic", k_panaInCart: "Pana In-Cart", k_cartRate: "cart rate",
  k_adsCost: "Ads Cost", k_roas: "ROAS",

  // Panels
  p_monthlySalesTitle: "Panasonic Monthly Sales", p_monthlySalesHint: "Penjualan Siap Dikirim per bulan · SPOS",
  ...
  p_storeMonthlyTitle: "Store Sales by Month", p_storeMonthlyHint: "All brands GMV · Performa",
  ...
  c_funnelPartialNote: "Funnel shape hidden — some stages have no data yet, so the proportions would be meaningless. Impression & Click are read from the newer SPOS template columns (Jumlah Produk Dilihat / Produk Diklik); they fill in as SPOS files are re-uploaded.",
  c_funnelNonMonotonicNote: "A later stage exceeds an earlier one. Periods uploaded with the older SPOS template have no true impression/click columns — those fall back to Halaman Produk Dilihat (page views) and Klik Pencarian (search clicks only), which undercounts real clicks.",

  // BaselineChart.tsx
  b_title: "Baseline vs Active Performance",
  b_loading: "Loading…",
  b_noBaseline: 'No "Month Awal" baseline data for {scope}.',
  ...
  b_scopeMonthHint: 'Pre-project snapshot ("Month Awal", {stores} store{storesPlural} total) vs {month}.',
  b_scopeAvgHint: 'Pre-project snapshot ("Month Awal", {stores} store{storesPlural} total) vs average across {months} month{monthsPlural} of data — city-wide totals, not per store. Pick a specific Month in the filter above to see that month\'s real numbers instead of an average.',
